...
const artboard = reactive(viewport.artboard);
...
watch(
  ...
  val => {
    if (val) {
      ...
      const iR = interpolateNumber(radius.value, radius.value * .9);
...
function generatePathPoints(x, y, width, height, cornerRadius, numPoints) {
  const points = [];
  const cr = Math.min(cornerRadius, width / 2, height / 2);

  // Function to add points along a quarter circle (corner)
  const addCornerPoints = (centerX, centerY, startAngle, pointsPerCorner) => {
  ...
  const straightPerimeter = perimeter - 4 * cornerPerimeter;
...
function generateColor(index) {
  // Create a gradient of colors for the orbs
  // Using a pastel palette similar to your image
  const colors = [
    "#ff0b5c", // pink
    "#a3a1ff", // purple
    "#96d4fa", // light blue
...
function drawSpotlight(ctx, x, y, radius) {
  ...
  draw(ctx => {
    ...
    if (box.value.target && orbs.value.length > 0) {
      ...
      orbs.value.forEach((orb, index) => {
        ...
        const radius = orb.iR(ease(clamp((now - orb.born) / 200)))  * props.volume;
