...
import { ADMIN_WALLET } from "@wearesage/shared";
...
async function loadShaderTomSketches() {
  ...
  try {
    ...
    const response = await fetch(`/api/sketches?walletAddress=${ADMIN_WALLET}&visibility=public`);
...
function getSketchById(id: string) {
  return sketches.value.find(sketch => sketch._id === id) || null;
}
