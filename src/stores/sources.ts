...
export const useSources = defineStore("sources", () => {
  ...
  const isNewStreamStable = computed(() => {
    return (
      rawStream.value > 0.001 && // Not just initialized to 0
      source.value !== null && // Source is actually set
      source.value !== AudioSource.MICROPHONE
    ); // Skip offset for real-time sources like microphone
  });
  ...
  async function performSourceCleanup(oldSource: AudioSource | null) {
    ...
    try {
      ...
      console.log(
        `🎵 Aggressively resetting queue for source switch (current tracks: ${queue.queue.tracks.length}, playing: ${queue.queue.isPlaying})`
      );
      queue.reset();
